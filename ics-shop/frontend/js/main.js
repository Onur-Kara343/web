...
async function loadBonusTools() {
    ...
    // Fallback Tools (werden von der API geladen, aber falls nicht, zeige alle als "Coming Soon")
    const tools = [
        { id: 'kopfarena', name: 'KopfArena', icon: 'fa-brain', description: '40+ Spiele die dein Gehirn trainieren – Logik, Aufmerksamkeit, Reaktion & mehr', available: false },
        { id: 'trueyou', name: 'TrueYou', icon: 'fa-chart-line', description: '15 Tests für deine tiefen psychologischen Eigenschaften – Schattenseiten, Bindungstyp, Big 5 & mehr', available: false },
        { id: 'emora', name: 'Emora', icon: 'fa-comment-dots', description: 'Emotionsrad mit 60+ Emotionen + Chatbot + Trigger-Landkarte für deine Gefühle', available: false },
        { id: 'calmmind', name: 'CalmMind', icon: 'fa-book', description: 'Tagebuch mit Kalenderansicht, ToDos, Briefe an dein Zukunfts-Ich & KI-Reflexion', available: false },
        { id: 'dreamweaver', name: 'Dreamweaver', icon: 'fa-moon', description: 'Traumtagebuch + KI-Analyse, Lucid Dream Tipps, Schlaf-Tracking & mehr', available: false }
    ];
    ...
    try {
        ...
        if (res.ok) {
            ...
            // Zeige alle Tools als "Coming Soon" (da noch nicht verfügbar)
            container.innerHTML = apiTools.map(tool => `
        ...
        // Fallback: Zeige alle als "Coming Soon"
...
// ============ FREE EBOOK DOWNLOAD (in Bibliothek) ============
async function handleFreeEbookDownload(event) {
    ...
    try {
        ...
        if (res.ok) {
            showNotification('Download-Link wurde an deine E-Mail gesendet!', 'success');
            form.querySelector('input[type="email"]').value = '';
        } else {
            showNotification(data.error || 'Fehler beim Download', 'error');
        }
    } catch (error) {
...
async function loadLibrary() {
    ...
    try {
        ...
        // Free eBooks Section (nur für eingeloggte User)
        ...
        console.error(error);
        container.innerHTML = '<div class="empty-library"><p class="error"><i class="fas fa-exclamation-triangle"></i> Fehler beim Laden der Bibliothek</p></div>';
    }
}
